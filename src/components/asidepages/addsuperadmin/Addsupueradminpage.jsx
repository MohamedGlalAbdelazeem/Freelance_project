...
function Addsupueradminpage() {
  ...
  const [managersPagenation, setmanagersPagenation] = useState([]);


  const schema = z
    .object({
      name: z.string().min(1, "الاسم يجب ان يكون 3 احرف على الاقل"),
      email: z .string() .min(1, { message: "البريد الالكترونى مطلوب" })  .email("البريد الالكترونى غير صحيح"),
...
const deleteManager = (id) => {
  ...
  axios
    ...
    .catch(function (error) {
    if (error.response.data.Message === "Not Found Or not Allow to Remove It") {
...
const fetchPagenation = () => {
  setLoader(true);
  axios
    .get(`http://127.0.0.1:8000/api/managers?page=${currentPage}`, {
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
    })
    .then(function (response) {
      setmanagersPagenation(response.data.data);
